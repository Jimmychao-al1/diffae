...
class UniformAffineQuantizer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # start quantization
        delta = self.delta if self.delta.device == x.device else self.delta.to(x.device)
        zp    = self.zero_point if self.zero_point.device == x.device else self.zero_point.to(x.device)
        x_int = round_ste(x / delta) + zp
        x_quant = self.clipping(x_int, 0, self.n_levels - 1) ## modified here to replace torch.clamp for gradient prop
        # x_quant = torch.clamp(x_int, 0, self.n_levels - 1)
        x_dequant = (x_quant - zp) * delta
        return x_dequant
...
class INT_UniformAffineQuantizer(nn.Module):
    """
    用於載入整數權重的量化器
    forward 只做反量化：x_float = (x_int - zero_point) * delta
    """
    def __init__(self, n_bits: int = 8, symmetric: bool = False, channel_wise: bool = False, 
                 scale_method: str = 'max', leaf_param: bool = False, weight_tensor=None):
        super(INT_UniformAffineQuantizer, self).__init__()
        self.sym = symmetric
        assert 2 <= n_bits <= 8, 'bitwidth not supported'
        self.n_bits = n_bits
        self.n_levels = 2 ** self.n_bits
        ...
        # 初始化 delta 和 zero_point（從 checkpoint 載入時會覆蓋）
        if weight_tensor is not None:
            if len(weight_tensor.shape) == 4:
                self.delta = nn.Parameter(torch.randn(size=(weight_tensor.shape[0], 1, 1, 1)))
                self.zero_point = nn.Parameter(torch.randn(size=(weight_tensor.shape[0], 1, 1, 1)))
    ...
    def forward(self, x_int: torch.Tensor):
        """
        直接反量化整數權重
        x_int: uint8 整數權重 (0 ~ n_levels-1)
        return: 反量化後的浮點數權重
        """
        # 確保 delta 和 zero_point 在同一設備上
        delta = self.delta if self.delta.device == x_int.device else self.delta.to(x_int.device)
        zp = self.zero_point if self.zero_point.device == x_int.device else self.zero_point.to(x_int.device)
        
        # 反量化：x_float = (x_int - zero_point) * delta
        # 注意：x_int 是 uint8，需要先轉為 float
        x_float = (x_int.float() - zp) * delta
        return x_float
...
class QuantModule(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.use_weight_quant:
            ...
            bias = self.org_bias.to(input.device)
...
class INT_QuantModule(nn.Module):
    """
    用於載入整數權重的量化模組
    weight 儲存為 uint8 整數，forward 時反量化為 float
    """
    def __init__(self, org_module: Union[nn.Conv2d, nn.Linear], weight_quant_params: dict = {},
                 act_quant_params: dict = {}, disable_act_quant: bool = False, se_module=None,  need_init=False):
        super(INT_QuantModule, self).__init__()
        ...
        # 量化狀態
        self.use_weight_quant = True  # INT 版本總是使用量化
        self.use_act_quant = False
        self.disable_act_quant = disable_act_quant
    ...
    def set_quant_state(self, weight_quant: bool = False, act_quant: bool = False):
        # INT 版本的 weight_quant 總是 True
        if not weight_quant:
            raise ValueError("INT_QuantModule 必須啟用 weight quantization")
...
#
        ...
        zp = self.zero_point if self.zero_point.device == x_int8.device else self.zero_point.to(x_int8.device)
        dl = self.delta      if self.delta.device      == x_int8.device else self.delta.to(x_int8.device)
        x_float_q = (x_int8.to(torch.float16) - zp) * dl
