...
from QATcode.quant_layer import QuantModule, StraightThrough
...
class SelectiveQuantModel(nn.Module):
    ...
    def quant_unet_modules(self, need_init: bool = True):
        """
        只對 UNet 的核心模組進行量化：
        - time_embed
        - input_blocks
...
def count_quantized_modules(model, target_modules=['input_blocks', 'middle_block', 'output_blocks', 'out']):
    """
    統計指定模組中的量化層數量
    """
    total_quant = 0
...
def analyze_unet_structure(model):
    """
    ...
    for module_name in non_unet_modules:
        if hasattr(model, module_name):
            ...
            conv_count = sum(1 for _, child in module.named_modules() if isinstance(child, nn.Conv2d))
            linear_count = sum(1 for _, child in module.named_modules() if isinstance(child, nn.Linear))
            
            if conv_count > 0 or linear_count > 0:
                logger.info("%s: %d Conv2d, %d Linear (跳過量化)", module_name, conv_count, linear_count)