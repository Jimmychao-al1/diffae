...
def _get_cli_args():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", default=False, help="僅跑最小路徑，不做 optimizer.step"
    )
    parser.add_argument("--max-steps", type=int, default=10, help="dry-run 模式下的最大步數")

    # 新增功能旗標（預設關閉 = zero-diff）
    parser.add_argument(
        "--quant-diag-interval", type=int, default=8, help="每隔多少個 epoch 記錄一次「每層有效量化更新比例」(<=0 表示關閉)"
    )
    parser.add_argument(
        "--quant-diag-topk", type=int, default=12, help="每次診斷時額外列出前 Top-K 層（依有效量化更新比例排序）"
    )
    parser.add_argument(
        "--quant-diag-root",
        type=str,
        default="QATcode/quantize_ver2/quant_diag_runs",
        help="量化更新診斷輸出根目錄（每次 run 自動建立新子資料夾）",
    )
    parser.add_argument(
        "--quant-diag-save-full-layers",
        action="store_true",
        default=False,
        help="是否在 jsonl 中保存每次診斷的完整 layers 資訊（檔案會較大）",
    )
    parser.add_argument(
        "--log-suffix", type=str, default="", help="log 檔名後綴，例如 1 -> step6_train_1.log"
    )
    parser.add_argument(
        "--auto-log", action="store_true", default=False, help="自動使用 timestamp 產生唯一 log 檔名"
    )
    parser.add_argument(
        "--tensorboard",
        action="store_true",
    ...
    parser.add_argument(
        "--tb-dir",
        type=str,
        default="QATcode/quantize_ver2/tb_runs",
    ...
    parser.add_argument(
        "--debug-scale-list-update",
        action="store_true",
        default=False,
        help="輸出 scale_list 參數 grad 與 step 前後變化（最小診斷，預設關閉）",
    )
    parser.add_argument(
        "--debug-scale-list-interval",
        type=int,
        default=100,
    ...
    parser.add_argument(
        "--debug-timestep-grad-steps",
        type=str,
        default="0,80,99",
        help="要診斷的 timestep，逗號分隔，例如 0,80,99",
    )
    parser.add_argument(
        "--debug-timestep-grad-interval",
        type=int,
        default=0,
    ...
    parser.add_argument(
        "--tail-repair-steps", type=int, default=1, choices=[1, 2], help="tail-repair 外層重複次數（1 或 2）"
    )
    parser.add_argument(
        "--tail-repair-t-range", type=str, default="0", help="要補強的 timestep，逗號分隔，例如 0 或 0,1,2,3,4"
    )
    parser.add_argument(
        "--tail-repair-lr-scale",
        type=float,
        default=0.25,
...
def _compute_effective_weight(module: QuantModule_DiffAE_LoRA) -> Optional[torch.Tensor]:
    ...
    if module.fwd_func is F.conv2d:
        lora_weight = module.loraB.weight.squeeze(-1).squeeze(-1) @ module.loraA.weight.permute(
            2, 3, 0, 1
        )
...
def collect_quant_update_diagnostics(
    ...
    - changed_ratio_same_scale: 固定使用 base a_w（觀察 LoRA 是否跨越既有量化臺階）
    """
    ...
    with torch.no_grad():
        for name, module in qnn.named_modules():
            ...
            if eff_w.shape != base_w.shape:
                LOGGER.warning(
                    f"[QuantUpdateDiag] skip layer={name}, shape mismatch {tuple(base_w.shape)} vs {tuple(eff_w.shape)}"
                )
                continue
            ...
            rows.append(
                {
                    ...
                    "mean_abs_delta_qnorm": float(
                        (q_eff_norm_dynamic - q_base_norm).abs().mean().item()
                    ),
                }
            )
    ...
    weighted_changed_ratio_dynamic = (
        sum(r["changed_ratio_dynamic"] * r["numel"] for r in rows) / total_numel
    )
    weighted_changed_ratio_same_scale = (
        sum(r["changed_ratio_same_scale"] * r["numel"] for r in rows) / total_numel
    )
...
def log_quant_update_diagnostics(
    ...
    """Public function log_quant_update_diagnostics."""
    ...
    LOGGER.info(
        ...
        total_epochs,
        s["layer_count"],
        s["weighted_changed_ratio_dynamic"],
        s["weighted_changed_ratio_same_scale"],
        s["mean_abs_delta_fp"],
        s["mean_abs_delta_qnorm"],
    )
    ...
    for row in diag["top_layers"]:
        LOGGER.info(
            ...
            row["name"],
            row["changed_ratio_dynamic"],
            row["changed_ratio_same_scale"],
            row["mean_abs_delta_fp"],
            row["mean_abs_delta_qnorm"],
            row["numel"],
        )
    return diag
...
def _plot_quant_diag_curves(history: List[Dict[str, Any]], run_dir: str) -> None:
    ...
    plt.plot(
        epochs, ratio_same, marker="s", linewidth=1.8, label="weighted_changed_ratio_same_scale"
    )
    plt.xlabel("Epoch")
    plt.ylabel("Ratio")
...
def get_adaptive_learning_rates(
    ...
    Args:
        ...
        act_quant_lr_override: 若給定，覆寫內建的 act_quant_lr（scale_list 參數羣組）
...
def setup_optimizer_with_dynamic_lr(
    ...
    for name, module in base_model.named_modules():
        if isinstance(module, QuantModule_DiffAE_LoRA):
            if not hasattr(module, "act_quantizer") or not hasattr(
                module.act_quantizer, "scale_list"
            ):
...
def main() -> "Any":
    ...
    try:
        ...
        if args.quant_diag_interval > 0:
            ...
            quant_diag_summary_json = os.path.join(
                quant_diag_run_dir, "quant_update_diag_summary.json"
            )
        ...
        quant_model: QuantModel_DiffAE_LoRA = create_quantized_model(
            diffusion_model, num_steps=CONFIG.NUM_DIFFUSION_STEPS, lora_rank=CONFIG.LORA_RANK
        )
        quant_model.cuda()
        quant_model.eval()
        ...
        eval_sample = base_model.conf._make_diffusion_conf(
        ...
        for epoch in range(CONFIG.NUM_EPOCHS):
            ...
            for rec in rollout_step_records:
                ...
                if tb_writer is not None:
                    tb_writer.add_scalar(
                        "loss/current_ddim_step_loss", float(rec["current_step_loss"]), _step_x
                    )
                    tb_writer.add_scalar(
                        "loss/running_mean_ddim_loss", float(rec["running_mean_ddim_loss"]), _step_x
                    )
            ...
            per_timestep_mean_loss[_valid] = (
                per_timestep_loss_sum[_valid] / per_timestep_loss_count[_valid]
            )
            ...
            if len(_tail_valid) > 0 and len(_all_valid) > 0:
                ...
                if tb_writer is not None:
                    ...
                    tb_writer.add_scalar(
                        "summary/tail_outlier_ratio", tail_outlier_ratio, _epoch_step
                    )
            ...
            LOGGER.info(
                "[LossSummary] per-timestep mean (first5): %s", _first5 if _first5 else "N/A"
            )
            ...
            for chunk in chunk_summaries:
                ...
                if tb_writer is not None:
                    tb_writer.add_scalar(
                        "chunk/mean_total_loss", chunk["mean_total_loss"], chunk_end_step
                    )
                    tb_writer.add_scalar(
                        "chunk/std_total_loss", chunk["std_total_loss"], chunk_end_step
                    )
                    tb_writer.add_scalar(
                        "chunk/max_total_loss", chunk["max_total_loss"], chunk_end_step
                    )
                    tb_writer.add_scalar(
                        "chunk/min_total_loss", chunk["min_total_loss"], chunk_end_step
                    )
                    if "mean_distill_loss" in chunk:
                        tb_writer.add_scalar(
                            "chunk/mean_distill_loss", chunk["mean_distill_loss"], chunk_end_step
                        )
                        tb_writer.add_scalar(
                            "chunk/std_distill_loss", chunk["std_distill_loss"], chunk_end_step
                        )
                        tb_writer.add_scalar(
                            "chunk/max_distill_loss", chunk["max_distill_loss"], chunk_end_step
                        )
                        tb_writer.add_scalar(
                            "chunk/min_distill_loss", chunk["min_distill_loss"], chunk_end_step
                        )
                ...
                if do_diag:
                    ...
                    if (quant_diag_run_dir is not None) and (diag.get("summary") is not None):
                        ...
                        quant_diag_history.append(
                            {
                                "epoch": int(epoch + 1),
                                "weighted_changed_ratio_dynamic": float(
                                    s["weighted_changed_ratio_dynamic"]
                                ),
                                "weighted_changed_ratio_same_scale": float(
                                    s["weighted_changed_ratio_same_scale"]
                                ),
            ...
            if improved:
                ...
                save_checkpoint(
                    ...
                    best_epoch,
                    best_loss,
                    ema_helper=distill_trainer.ema,
                    is_best=True,
                )
            ...
            if epoch % 8 == 0:  # 減少頻率，節省時間
                ...
                with torch.no_grad():
                    ...
                    # 小批量驗證生成
                    val_batch_size = 8
                    x_T = torch.randn((val_batch_size, 3, 128, 128), device=CONFIG.DEVICE)
                    ...
                    if hasattr(ema_model, "set_runtime_mode"):
                        ema_model.set_runtime_mode(
                            mode="infer", use_cached_aw=True, clear_cached_aw=True
                        )
                    for name, module in ema_model.named_modules():
                        if hasattr(module, "act_quantizer") and hasattr(
                            module.act_quantizer, "current_step"
                        ):
                    ...
                    LOGGER.info(
                        f"正規化後數值範圍: [{batch_images_normalized.min():.4f}, {batch_images_normalized.max():.4f}]"
                    )

                    # 創建網格圖像
                    combined_images = torch.cat(
                        [fp_images_normalized, batch_images_normalized], dim=0
                    )
                    grid_image = torchvision.utils.make_grid(
                        combined_images, nrow=val_batch_size, padding=2
                    )
                    torchvision.utils.save_image(
                        grid_image,
                        os.path.join(
                            "QATcode/quantize_ver2/training_samples", f"epoch_{epoch}_grid.png"
                        ),
                    )
