"""Step4 selective quantization workflow and calibration diagnostics."""
...
from QATcode.quantize_ver2.quant_model_selective_v2 import (
    SelectiveQuantModel,
...
def _compute_layer_stats_cpu(task):
    ...
    pre = (
        _np.asarray(pre_arr, dtype=_np.float64)
        if pre_arr is not None
        else _np.asarray([], dtype=_np.float64)
    )
    post = (
        _np.asarray(post_arr, dtype=_np.float64)
    ...
    try:
        ...
        if nb is not None and act_delta is not None:
            ...
            if post.size > 0:
                errors["pct_clipped"] = float(
                    ((post < rep_min_f) | (post > rep_max_f)).sum()
                ) / float(post.size)
            if pre.size > 0:
                try:
...
class ActivationCollector:
    """Public class ActivationCollector."""
    ...
    def _make_hook(self, name):
        def _hook(module, inp, out):
            ...
            try:
                ...
                if self.per_channel_sample and td.dim() >= 3:
                    if td.dim() >= 4:
                        ...
                        for c in range(C):
                            ...
                            clen = vec.numel()
                            k = min(
                                self.per_channel_sample,
                                clen,
                                max(1, int(self.sample_size // max(1, C))),
                            )
                            if clen <= k:
                                samp = vec.reshape(-1)
                            else:
                ...
                LOGGER.warning("Activation reservoir collection failed for layer '%s': %s", name, e)
            ...
            try:
                ...
                LOGGER.warning(
                    "Activation quantizer metadata collection failed for layer '%s': %s", name, e
                )
            ...
            try:
                    ...
                    if wdelta is not None and "weight_delta_count" not in mmeta:
                        if torch.is_tensor(wdelta):
                            try:
                                ...
                                q1 = float(
                                    torch.quantile(wdev, torch.tensor(0.25, device=self._device))
                                    .detach()
                                    .cpu()
                                    .item()
                                )
                                q2 = float(
                                    torch.quantile(wdev, torch.tensor(0.5, device=self._device))
                                    .detach()
                                    .cpu()
                                    .item()
                                )
                                q3 = float(
                                    torch.quantile(wdev, torch.tensor(0.75, device=self._device))
                                    .detach()
                                    .cpu()
                                    .item()
                                )
                                ...
                                for q in self.quantiles:
                                    try:
                                        extra_q[str(q)] = float(
                                            torch.quantile(
                                                wdev, torch.tensor(q, device=self._device)
                                            )
                                            .detach()
                                            .cpu()
                                            .item()
                                ...
                                mmeta.update(
                                    {
                                        ...
                                        "weight_delta_mean": mean,
                                        "weight_delta_std": std,
                                        "weight_delta_min": mn,
                                        "weight_delta_max": mx,
                                ...
                                try:
                                    ...
                                    extra_q = {
                                        str(q): float(np.quantile(wf, q)) for q in self.quantiles
                                    }
                                    mmeta.update(
                                        {
                                            "weight_delta_q1": float(q25),
                                            "weight_delta_q2": float(q50),
                                            "weight_delta_q3": float(q75),
                                            "weight_delta_quantiles": extra_q,
                                            "weight_delta_mean": float(np.mean(wf)),
                                            "weight_delta_std": float(np.std(wf)),
                                            "weight_delta_min": float(np.min(wf)),
                                            "weight_delta_max": float(np.max(wf)),
                            ...
                            try:
                                ...
                                extra_q = {
                                    str(q): float(np.quantile(arr, q)) for q in self.quantiles
                                }
                                mmeta.update(
                                    {
                                        "weight_delta_q1": float(q25),
                                        ...
                                        "weight_delta_q3": float(q75),
                                        "weight_delta_quantiles": extra_q,
                                        "weight_delta_mean": float(np.mean(arr)),
                                        "weight_delta_std": float(np.std(arr)),
                                        "weight_delta_min": float(np.min(arr)),
                                        "weight_delta_max": float(np.max(arr)),
                ...
                LOGGER.warning("Weight delta summary collection failed for layer '%s': %s", name, e)
    ...
    def dump_results(self, out_dir: "Any" = None, n_workers: "Any" = 10) -> "Any":
        ...
        for base in base_names:
            ...
            pre_arr = (
                np.concatenate(pre_list) if len(pre_list) > 0 else np.asarray([], dtype=np.float64)
            )
            post_arr = (
                np.concatenate(post_list)
                if len(post_list) > 0
        ...
        for base in topk_layers:
            ch_keys = sorted(
                [
                    k
                    for k in set(list(self.buff_pre.keys()) + list(self.buff_post.keys()))
                    if k.startswith(base + "__ch")
                ]
            )
            if not ch_keys:
                continue
            ...
            for ch_key in ch_keys:
                ...
                try:
                    ...
                    if nb is not None and act_delta is not None:
                        if isinstance(act_delta, (list, tuple, np.ndarray)):
                            ...
                            try:
                                ...
                                ch_idx = None
                            delta_c = (
                                float(np.mean(act_delta))
                                if ch_idx is None
                                else float(act_delta[ch_idx])
                                if ch_idx < len(act_delta)
                                else float(np.mean(act_delta))
                            )
                        else:
                            delta_c = float(act_delta)
                    ...
                    LOGGER.warning(
                        "Per-channel error statistic calculation failed for layer '%s' channel '%s': %s",
                        base,
                        ch_key,
                        e,
                    )
                per_channel_results[base][ch_key.split("__ch")[-1]] = {
                    "pre": pre_stat,
                    "post": post_stat,
                    "errors": errors,
                }
    ...
    def close(self) -> "Any":
        ...
        for h in self._handles:
            try:
                ...
                LOGGER.warning("Failed to remove hook handle: %s", e)
        self._handles = []
...
def calibrate_quantized_model(
    ...
    collector = ActivationCollector(
        model=qnn, out_dir="./analysis_step4", run_tag="step4_diffae"
    )  # change run_tag to "step4_efficientdm" when running EfficientDM script
    collector.set_mode("pre")
    qnn.set_quant_state(
        True, False
    )  # disable act fake-quant but keep weight quant (if that API toggles that way)
    qnn.eval()

    with torch.no_grad():
        print("First run to init quantization parameters (latent cond if available)...")
        _ = qnn(
            x=cali_images[:32].to(device), t=cali_t[:32].to(device), cond=cali_y[:32].to(device)
        )
...
def main() -> "Any":
    ...
    try:
        ...
        model.load_state_dict(
            torch.load(f"{conf.logdir}/last.ckpt", map_location="cpu", weights_only=False)[
                "state_dict"
            ]
        )
        ...
        # 4. 創建量化模型
        qnn, total_quant = create_quantized_model(
            model, wq_params, aq_params, quantize_skip_connections=False
        )
        # print(qnn)
