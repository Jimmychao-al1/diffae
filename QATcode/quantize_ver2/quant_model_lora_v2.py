...
class QuantModule_DiffAE_LoRA(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "weight",
            torch.randn(size=[self.ori_shape[0] // self.size_scale] + list(self.ori_shape[1:])),
        )
        ...
        self.weight_quantizer = UniformAffineQuantizer(
            **weight_quant_params, weight_tensor=org_module.weight
        )
    ...
    def forward(self, input: torch.Tensor) -> Any:
        ...
        if self.fwd_func is F.linear:
            ...
            lora_weight = self.loraB.weight.squeeze(-1).squeeze(-1) @ self.loraA.weight.permute(
                2, 3, 0, 1
            )
        ...
        if self.use_weight_quant and self.use_act_quant:
            ...
            step_idx = max(
                0, min(self.act_quantizer.current_step, self.act_quantizer.total_steps - 1)
            )
    ...
    def quant_unet_modules_with_lora(
        ...
        for module_name in self.target_modules:
            if hasattr(self.model, module_name):
                ...
                # 遞歸替換量化層
                self.quant_module_refactor_with_lora(module, weight_quant_params, act_quant_params)
            else:
                ...
                if not should_skip:
                    ...
                    if self.count in self.special_module_count_list:
                        ...
                        setattr(
                            ...
                            name,
                            QuantModule(
                                child_module, weight_quant_params, act_quant_params, need_init=False
                            ),
                        )
                    else:
            ...
            if isinstance(child, (nn.Conv2d, nn.Linear)):
                ...
                if self.count in self.special_module_count_list:
                    ...
                    setattr(
                        module,
                        name,
                        QuantModule(child, weight_quant_params, act_quant_params, need_init=False),
                    )
                else:
