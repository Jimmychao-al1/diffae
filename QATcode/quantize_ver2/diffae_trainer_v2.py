...
class SpacedDiffusionBeatGans_Trainer(SpacedDiffusionBeatGans):
    ...
    def _run_timestep_grad_conflict_debug(
        ...
        steps = [int(s) for s in self.debug_timestep_grad_steps if 0 <= int(s) < int(self.num_timesteps)]
    ...
    def pop_completed_loss_chunks(self):
        """取出並清空本輪新產生的 chunk summaries。"""
        chunks = self._completed_loss_chunks
        self._completed_loss_chunks = []
        return chunks

    def pop_rollout_step_records(self):
        ...
        for i in indices:
            ...
            if self.step_count % 20 == 0:
                LOGGER.info(
                    ...
                    self.step_count,
                    int(i),
                    float(out["current_step_loss"]) if out.get("current_step_loss") is not None else float("nan"),
                    float(out["running_mean_ddim_loss"]) if out.get("running_mean_ddim_loss") is not None else float("nan"),
    ...
    def training_losses_with_inference_distillation(self, 
                                                   ...
                                                   debug_epoch: Optional[int] = None) -> Dict[str, torch.Tensor]:
        """
