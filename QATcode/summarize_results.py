...
def summarize_results(results_dir="QATcode"):
    ...
    for steps in sorted(df['steps'].unique()):
        ...
        for method in sorted(df[df['steps'] == steps]['method'].unique()):
            ...
            for _, row in subset.iterrows():
                ...
                # 显示三个时间：Original / Quantized / Quantized+Cache
    ...
    if len(cache_df) > 0:
        ...
        print(f"最快生成时间: {cache_df.loc[min_time_idx, 'quantized_cache_time_s']:.3f}s "
    ...
    if len(cache_reduction_df) > 0:
        ...
        print(f"最大 Cache 减少: {cache_reduction_df.loc[max_reduction_idx, 'cache_reduction_ratio']:.1f}% "
