...
class SimilarityCollector:
    ...
    def __init__(
        ...
        for d in [self.result_npz_dir, self.l1_dir, self.l1_change_dir, self.l2_dir, self.cos_dir, self.tier_plot_dir]:
    ...
    def finalize(self):
        """輸出每個 block 的 npz + png，以及 tier 聚合圖。"""
        for block_name in self.block_sums.keys():
            ...
            l2_step_mean, l2_step_std = self._step_mean_std(block_name, "l2")
            ...
            np.savez_compressed(
                ...
                l2rel=l2_mean.astype(self.save_dtype),
                ...
                l2_step_mean=l2_step_mean.astype(self.save_dtype),
                l2_step_std=l2_step_std.astype(self.save_dtype),
            ...
            l1_change_block_dir = self.l1_change_dir / block_slug
            l2_block_dir = self.l2_dir / block_slug
            cos_block_dir = self.cos_dir / block_slug
            for d in [l1_block_dir, l1_change_block_dir, l2_block_dir, cos_block_dir]:
                d.mkdir(parents=True, exist_ok=True)

            self._save_matrix_csv(l1_mean, l1_block_dir / f"{block_slug}_l1rel.csv")
            self._save_matrix_csv(l1_rate_mean, l1_change_block_dir / f"{block_slug}_l1rel_rate.csv")
            self._save_matrix_csv(l2_mean, l2_block_dir / f"{block_slug}_l2rel.csv")

            # 繪製單組折線圖（原有功能）
            self._plot_step_curve(l1_step_mean, l1_step_std, l1_block_dir / f"{block_slug}_l1.png", "L1rel")
            self._plot_step_curve(l1_rate_step_mean, l1_rate_step_std, l1_change_block_dir / f"{block_slug}_l1_rate.png", "L1rel Rate")
            self._plot_step_curve(l2_step_mean, l2_step_std, l2_block_dir / f"{block_slug}_l2.png", "L2rel")
            ...
            if block_name in self.batch_step_data and len(self.batch_step_data[block_name]) > 0:
                ...
                self._plot_multi_batch_curves(
                    block_name, "l1_rate", l1_change_block_dir / f"{block_slug}_l1_rate_multi_batch.png", "L1rel Rate (Multi-Batch)"
                )
                self._plot_multi_batch_curves(
                    block_name, "l2", l2_block_dir / f"{block_slug}_l2_multi_batch.png", "L2rel (Multi-Batch)"
                )
            self._plot_heatmap(cos_mean, cos_block_dir / f"{block_slug}_cosine_heatmap.png", "Cosine similarity")
            if self.save_cosine_step_plot:
                self._plot_step_curve(cos_step_mean, cos_step_std, cos_block_dir / f"{block_slug}_cosine.png", "Cosine")
...
def main_float_model():
    ...
    try:
        ...
        if CONFIG.ENABLE_CACHE_ANALYSIS:
            ...
            similarity_root = f"{CONFIG.SIMILARITY_OUTPUT_ROOT}/T_{T}/Res"
