...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../a_L1_L2_cosine")))
...
class SvdFeatureCollector:
    ...
    def register_hooks(self, model: nn.Module, sampler: "Any") -> "Any":
        ...
        for name, module in model.named_modules():
            ...
            if (
                name == self.target_block
                or f"model.{name}" == self.target_block
                or name == self.target_block.replace("model.", "")
            ):
...
def main() -> "Any":
    ...
    try:
        ...
        LOGGER.info(
            f"[SVD] eval_num_images 設為 {conf.eval_num_images}（{num_batches_needed} batches）"
        )
        ...
        if CONFIG.IN_MEMORY_PIPELINE:
            ...
            if not CONFIG.SKIP_CORRELATION:
                ...
                from QATcode.cache_method.b_SVD.correlate_svd_similarity import (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--svd_target_block", type=str, required=True, help="目標 block 名稱（如 model.input_blocks.0）"
    )
    parser.add_argument("--svd_target_N", type=int, default=32, help="每個 timestep 收集的樣本數")
    parser.add_argument(
        "--svd_output_root", type=str, default="QATcode/cache_method/b_SVD", help="輸出根目錄"
    )
    parser.add_argument("--log_file", "--lf", type=str, default=None, help="Log 檔案路徑")
    parser.add_argument(
        "--in_memory_pipeline", action="store_true", help="A 收集後直接在記憶體執行 B/C，不寫 t_{t}.pt"
    )
    parser.add_argument(
        "--representative-t", type=int, default=-1, help="B 階段 rank 估計代表 timestep，-1 表示最後一步"
    )
    parser.add_argument(
        "--energy-threshold", type=float, default=0.98, help="B 階段 cumulative energy 門檻"
    )
    parser.add_argument("--no-compute-energy", action="store_true", help="B 階段不計算 energy ratio")
    parser.add_argument(
        "--similarity_npz",
        type=str,
        default=None,
