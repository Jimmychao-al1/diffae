...
def load_diffae_model(model_path: str = CONFIG.MODEL_PATH) -> LitModel:
    ...
    Returns:
        LitModel: 加載的擴散模型
    """
    return _common_load_diffae_model(
        model_path,
        LOGGER,
        log_on_start="載入原本 Diff-AE 模型: %s",
        log_on_done="✅ 原本 Diff-AE 模型載入完成",
    )
...
def load_results(json_path: str) -> dict:
    ...
    if os.path.exists(json_path):
        ...
        return {
            "config": {"eval_samples": CONFIG.EVAL_SAMPLES, "model_type": "original_diffae"},
            "results": {},
        }
...
def evaluate_fid_with_train(
    cache_scheduler: Optional[dict] = None, num_steps: int = None, gpus: List[int] = None
) -> float:
    """
    ...
    dummy = DataLoader(
        TensorDataset(torch.tensor([0.0] * conf.batch_size)), batch_size=conf.batch_size
    )
        ...
        if use_train_mode:
            ...
            if baseline_only:
                ...
                if "baseline_fid" not in results["results"][step_config]:
                    baseline_fid = evaluate_fid_with_train(
                        cache_scheduler=None, num_steps=CONFIG.NUM_DIFFUSION_STEPS, gpus=gpus
                    )
            ...
            for i, layer in enumerate(layers_to_test, 1):
                ...
                fid = evaluate_fid_with_train(
                    cache_scheduler=cache_config, num_steps=CONFIG.NUM_DIFFUSION_STEPS, gpus=gpus
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_train", action="store_true", help="使用 train() 函數方式（類似 run_ffhq128.py），不需要預先載入模型"
    )
    parser.add_argument(
        "--gpus", type=str, default="0", help="GPU 列表（逗號分隔，例如: 0,1），僅在 --use_train 模式下使用"
    )
