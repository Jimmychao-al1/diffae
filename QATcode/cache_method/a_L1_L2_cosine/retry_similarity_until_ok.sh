...
# ========= 你的目標指令 =========
CMD='bash QATcode/cache_method/a_L1_L2_cosine/run_similarity_experments.sh'

# ========= 自動定位專案根目錄 =========
# retry.sh 放在：QATcode/cache_method/a_L1_L2_cosine/ 底下
# 往上 3 層就是專案根目錄
WORKDIR="$(cd "$(dirname "$0")/../../.." && pwd)"

# ========= log 位置 =========
LOG_DIR="${WORKDIR}/QATcode/cache_method/a_L1_L2_cosine/retry_logs_similarity"
mkdir -p "$LOG_DIR"
...
while true; do
  ...
  # 2) 確認目標腳本存在（避免路徑打錯）
  TARGET_SH="${WORKDIR}/QATcode/cache_method/a_L1_L2_cosine/run_similarity_experments.sh"
  if [ ! -f "$TARGET_SH" ]; then
