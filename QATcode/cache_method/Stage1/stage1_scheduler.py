"""
Stage-1 baseline（新版）：global shared zones + cost-based per-block k + expanded_mask
...
def processing_order_series(G_ddim: np.ndarray, T: int) -> np.ndarray:
    """G_proc[i] = G_ddim[t=99-i]，長度 T；i=0 為第一步 t=99。"""
...
def run_stage1_synthesis(
    ...
    # --- expanded_mask [B, T]，步序 i：i=0 -> t=99 ---
...
def self_test():
    ...
    np.save(tmp / "fid_w_qdiffae_clip.npy", rng.random(B).astype(np.float64))
