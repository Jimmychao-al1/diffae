...
def compute_energy_ratios(eigenvalues_list: List[torch.Tensor], k_values: List[int] = [4, 8, 16, 32]) -> Dict:
    """
...
def main():
    ...
    parser.add_argument('--feature_dir', type=str, help='單一 block 的 feature 目錄（svd_features/<block_slug>/）')
    parser.add_argument('--feature_root', type=str, default='QATcode/cache_method/SVD/svd_features',
                        help='svd_features 根目錄（批次處理所有 block）')
    parser.add_argument('--output_root', type=str, default='QATcode/cache_method/SVD/svd_metrics',
                        help='輸出根目錄')
    parser.add_argument('--representative-t', type=int, default=-1,
                        help='代表 timestep（用於定 rank r），-1 表示 T-1（預設）')
    parser.add_argument('--energy-threshold', type=float, default=0.98,
                        help='Cumulative energy 門檻（預設 0.98）')
    parser.add_argument('--compute-energy', action='store_true', default=True,
                        help='計算 energy ratio 曲線')
    parser.add_argument('--all', action='store_true',
                        help='批次處理 feature_root 下所有 block')
