...
├── run_svd_feature_experiments.sh   # Shell：31 個 block 批次收集
...
bash QATcode/cache_method/SVD/run_svd_feature_experiments.sh
```
...
**目的**：分析 SVD 子空間距離與 similarity（L1rel、Cosine）在 timestep 維度上的相關性。
...
python QATcode/cache_method/SVD/correlate_svd_similarity.py \
  ...
  --similarity_npz QATcode/cache_method/L1_L2_cosine/T_100/Res/model_input_blocks_0.npz \
...
python QATcode/cache_method/SVD/correlate_svd_similarity.py \
  ...
  --similarity_npz_dir QATcode/cache_method/L1_L2_cosine/T_100/Res \
  --output_root QATcode/cache_method/SVD/correlation \
  --plot \
  --all
```
...
- **與 similarity 的差異**：
  - Similarity：累積後計算 L1/L2/cosine，寫 NPZ/CSV/PNG
  - SVD：只累積 raw feature，寫 `t_{t}.pt` + `meta.json`
- **GPU 優化**：
  - Hook 內只做 `detach()` 與 append，feature 保持在 GPU
  - Finalize 時在 GPU 上做 concat + 截斷，寫檔前才搬 CPU
- **target_N**：每個 t 最終寫出 `(target_N, C, H, W)`，所有 timestep 共用同一組 N 個 sample

### 2. SVD 指標（階段 B）

- **Covariance**：對 `(N, C, H, W)` reshape 成 `(C, M)`（M=N×H×W），計算 `Σ = (X @ X^T) / M`
...
- **序列對齊**：t=1..T-1（跳過 t=0，因為 SVD dist[0]=0）
- **相關性**：Pearson / Spearman（L1 vs SVD、L2 vs SVD、CosDist vs SVD）
...
```bash
# 1. 收集所有 block 的 feature（約需數小時，31 個 block × 每個約 2-3 分鐘）
bash QATcode/cache_method/SVD/run_svd_feature_experiments.sh
...
編輯 `run_svd_feature_experiments.sh`，修改 `TARGET_N=32` 為 `TARGET_N=128`。
